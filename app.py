...
with st.sidebar:
    ...
    if selected == "Home":
        st.title("Discover your next favorite movie with our personalized recommendations.")
    if selected == "About":
        st.title("Hey my name is Rizwan Ahamad\n")
        s = option_menu(
            menu_title=None,
        ...
        if s == "github":
            st.title("You Can Check My Github\nhttps://github.com/Rizwan9451")
        elif s == "linkedin":
            st.title("You Can Check My Linked\nhttps://www.linkedin.com/in/rizwan-ahamad-4814a8221/")

def crew(movie_id):
    response = requests.get(
        "https://api.themoviedb.org/3/movie/{}/credits?api_key=047b5d937e9c61972de4800ecc7ba3f1&language=en-US".format(
            movie_id))
    data = response.json()
    crew_name = []
    final_cast = []
...
def overview(movie_id):
    response = requests.get(
        "https://api.themoviedb.org/3/movie/{}?api_key=6b536f5663d08f682be8eeb7051be2cc&language=en-US".format(
            movie_id))
    data = response.json()
